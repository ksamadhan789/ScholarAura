...
- Recruiter plans (2026-09-25): the refund policy (/refund-policy) doesn't mention Pro plan payments (or job Boosts). Decide the terms (e.g. refundable within N days if unused, or non-refundable) and add them there — the plan page deliberately makes no refund promise until then. There's no admin refund button for plan payments yet either; refund from the Razorpay dashboard and mark the row REFUNDED if ever needed.
