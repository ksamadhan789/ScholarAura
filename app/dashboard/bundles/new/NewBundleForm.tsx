...
export function NewBundleForm({ courses }: { courses: CourseOption[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data?.error ?? "Couldn't create the bundle. Please try again.");
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      >
        {loading ? "Creating…" : "Create bundle (as draft)"}
      </button>
    </form>
  );
}
