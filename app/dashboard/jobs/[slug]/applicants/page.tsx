...
export default async function JobApplicantsPage({ params }: { params: { slug: string } }) {
    ...
    <main className="mx-auto max-w-3xl px-4 py-16">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">{job.companyName}</p>
        </div>
          ...
          {applications.map((app) => (
            ...
            >
              ...
              <div className="mt-3 flex items-center gap-4">
                ...
                <Link
                  href={`/dashboard/jobs/${job.slug}/applicants/${app.id}/messages`}
                  className="text-sm text-brand-600 underline dark:text-brand-400"
                >
                  💬 Message
                </Link>
