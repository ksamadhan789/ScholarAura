...
const updateCompetitionSchema = z
  .object({
    ...
    submissionDeadline: z.preprocess(fromIstInput, z.coerce.date()).optional(),
...
export async function GET(
  ...
  if (!competition || (!competition.isPublished && !isAdmin)) {
    return NextResponse.json({ error: "Competition not found" }, { status: 404 });
  }

  return NextResponse.json(isAdmin ? competition : toPublicListing(competition));
}
...
export async function PATCH(
  ...
  if (!competition.isPublished && updated.isPublished) {
    await notifyInterestedStudents("competition", {
      slug: updated.slug,
      title: updated.title,
