...
export function BundleEnrollButton({
  slug,
  isPaid,
  price,
  rates,
  ...
  async function handleFreeEnroll() {
    const res = await fetch(`/api/bundles/${slug}/enroll`, { method: "POST" });
    if (!res.ok) {
  ...
  async function handlePaidEnroll() {
    const checkoutRes = await fetch(`/api/bundles/${slug}/checkout`, {
      method: "POST",
    ...
    const razorpay = new window.Razorpay({
      ...
      handler: async (response) => {
        const verifyRes = await fetch(`/api/bundles/${slug}/verify-payment`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
