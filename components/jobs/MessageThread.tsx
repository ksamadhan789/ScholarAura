...
export function MessageThread({
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
              ...
              <div key={m.id} className={`flex max-w-[80%] items-end gap-2 ${isMine ? "self-end" : "self-start"}`}>
                {!isMine && (
                  <Avatar
                    ...
                    src={
                      m.senderPhotoFileId
                        ? `/api/job-applications/${applicationId}/messages/photo?userId=${m.senderId}`
                        : null
                    }
                    size={28}
                  />
                ...
                >
                  ...
                  <p className={`mt-1 text-xs ${isMine ? "text-white/70" : "text-gray-500 dark:text-slate-400"}`}>
      ...
      >
        <label htmlFor={`message-${applicationId}`} className="sr-only">
          Your message
        </label>
        <textarea
