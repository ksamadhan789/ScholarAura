...
### Added
- Competition entries admin page (`/dashboard/competitions/[slug]/entries`) gets a "Download entry files (ZIP)" button next to "Export CSV" (shown only when at least one entry has a file) — new `GET /api/admin/competitions/[slug]/entries/bulk-zip` downloads every submitted entry file for that competition from Drive and streams back one ZIP, mirroring the existing certificates bulk-zip route/pattern (bounded concurrency, one shared `JSZip`). Previously the only way to get all entry files at once was manually selecting them in Google Drive.
- Certificate config on event/competition edit pages gets a Certify'em-inspired polish (same underlying settings, no schema change): the `certificateEnabled` checkbox is now a "Status: Automatic Certification is ON/OFF" box with a "Turn ON/OFF" button; the Google Slides template ID field gets a "Preview template ↗" link that opens the actual Slides doc; and a new "📧 Email me a sample" button generates a real sample certificate (fake placeholder data, same Slides→PDF→QR-stamp pipeline as a real one, never saved as a `Certificate` row or kept in Drive) and emails it as a PDF attachment via a new `sendCertificateSampleEmail()`. New `generateSampleCertificatePdf()` (`lib/certificateGeneration.ts`) and `POST /api/{events,competitions}/[slug]/certificate-sample` (admin-only) routes.
- Mobile header now shows a compact "Sign up/in" button next to the theme toggle when logged out (`components/Header.tsx`) — previously a mobile visitor had to open the hamburger menu to find login/register; desktop already showed them inline and is unaffected.
...
### Added
- Student ID card and competition entry file uploads now properly support files over Vercel's ~4.5MB request-body limit, using **Vercel Blob's client-upload SDK** (`@vercel/blob`) instead of the reverted direct-to-Drive attempt from earlier today. The browser calls `upload()` from `@vercel/blob/client` to send the file straight to Blob storage (private access) — a new token-issuing route per upload (`POST /api/account/id-card/blob-upload`, `POST /api/competitions/[slug]/submit/blob-upload`) checks auth/eligibility and calls `handleUpload()` server-side before Blob will accept the upload. The browser then POSTs only the resulting `blobUrl` (small JSON, nowhere near the body-size cap) to the existing confirm routes (`POST /api/account/id-card`, `POST /api/competitions/[slug]/submit`), which now accept JSON instead of multipart `FormData`. New `lib/blobUpload.ts` (`downloadBlobBytes()`, `deleteBlob()`) pulls the bytes back down server-side using Blob's `get()`, so the same magic-bytes/size validation and Drive upload (`uploadFile()`) run exactly as before — Blob is purely temporary staging, deleted right after the bytes reach Drive. `MAX_UPLOAD_BYTES` (`lib/uploadValidation.ts`) is back to 25MB. Unlike the reverted attempt, this was verified by reading the actual installed `@vercel/blob` package's type definitions directly before writing any code, rather than assuming API behavior from memory — Blob is also a first-party Vercel product with genuine CORS support for this exact use case, not a third-party API being used off-label. `SubmissionForm.tsx` and `EditProfileForm.tsx` both show live upload-percentage progress via `onUploadProgress`. **Requires `BLOB_READ_WRITE_TOKEN`** to be set in the Vercel project (provisioned automatically once Vercel Blob storage is added to the project) — without it, uploads to these two routes will fail with a clear "couldn't start the upload" error rather than a silent drop.

### Fixed
- Reverted the direct-to-Drive browser upload (the same-day change that removed the 4MB limit) after it broke real entry submissions in production — confirmed live via a "Couldn't reach the server" error, then root-caused with a manual CORS preflight check against Google's endpoints: the Drive resumable-upload *session-creation* endpoint returns proper `Access-Control-Allow-Origin` headers, but the *upload-continuation* endpoint the browser actually PUTs bytes to (a different backend — `UploadServer` vs `ESF`) returns none at all, so the browser silently blocks that PUT. The assumption that "Google's resumable endpoint supports CORS for this" was only true for the first step, not the one that matters. Reverted `lib/google/driveService.ts`, both storage libs, both upload routes, and both client forms back to uploading bytes through our own server (multipart, same as before today's direct-upload attempt) — `MAX_UPLOAD_BYTES` is back to 4MB. Deleted the now-broken `createResumableUploadSession()`, the two `.../upload-session` routes, and `lib/directUpload.ts`. All the other same-day UX changes (entry link field back alongside file upload, inline ID card section on the entry page, "Competition entry link" label) are kept — only the transport mechanism reverted. A real fix for uploads over 4MB needs a proven bypass (e.g. Vercel Blob's client-upload SDK, purpose-built for this) verified against a live deployment before shipping again, not another assumption about a third-party API's CORS behavior.
