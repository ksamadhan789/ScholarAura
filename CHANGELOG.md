...
- Aura got a friendlier avatar — first a person-silhouette icon instead of a 🤖 robot emoji, then changed again to a graduation cap (`components/aura/AuraAvatar.tsx`, shared by the floating widget and the full `/aura` page) echoing the ScholarAura logo mark (`public/favicon-mark.png`), so the widget reads as part of the site's own identity rather than a generic chatbot or person.
